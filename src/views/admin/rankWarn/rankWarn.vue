...
<master-page title="实验开出预警">
    <div class="dashboard-container ">
      <div class="search-select">
        <el-button type="primary" icon="el-icon-download" href='javascript:;' @click="handleDownload">导出</el-button>
        ...
        <el-table v-show="changeSchool" :data="schoolList" fit highlight-current-row style="width: 100%" ref="editTable">
          <el-table-column prop="unitName" align="left" label="学校名称" min-width="250">
            <template slot-scope="scope">
              <router-link :to="{name: 'ConditSchool',params:{id:scope.row.unitId,name:scope.row.unitName}}" class="el-button el-button--text">
...
export default {
  ...
  methods: {
    getSchoolList() {
      ...
      getSchoolCondit(this.searchData).then(res => {
        this.schoolList = res;
        var that = this;
        setTimeout(function() {
          //设置背景色
          $("td.bgColor").each(function(index, element) {
            var classDate = Date.parse(
              new Date(that.schoolList[index].classDate)
            );
    ...
    handleDownload() {
      require.ensure([], () => {
        ...
        const list = JSON.parse(JSON.stringify(this.schoolList));
        list.forEach(function(s) {
          s.classDate = s.classDate.slice(0,10);
        });
        var name = changeDate(new Date()) + "实验开出预警";
