      ...
      <div class="link-source" v-if="list&&list.length>0">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              <span >相关资源</span>
              <el-button type="text" class="fr" v-if="list.length>=6" >更多</el-button>
            </div>
...
<script>
  ...
  export default {
    ...
    created() {
      this.content = this.$store.state.commonData.editPreviewData;

    },
    methods:{
       getList(){
         console.log(this.content.catalogueId);
         getLinkSource({catalogueId:this.content.catalogueId}).then(res=>{
           console.log(res);
           this.list=res;
         })
      },
