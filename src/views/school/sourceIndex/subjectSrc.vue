                  ...
                  <el-card :body-style="{ padding: '0px',clear:'both',height:'0'}">
                    ...
                    <div class="panelBody" >
                      ...
                      <p class="nav"  >
                        ...
                        <span @click="subjectGetList" disabled  >{{subjectName}}</span>
...
<script>
  ...
  export default {
    ...
    watch: {
    	sourceName() {
    		this.searchData.sourceName = this.sourceName;
    		this.getList();
    	},
    	subjectId() {
      ...
      this.searchData.textbookCode = this.subjectId;
       ...
       objTemp&&getInnerSourceSubject(objTemp).then(res=>{
            this.sourceSubject=res;
             this.sourceSubject&& this.sourceSubject.forEach(e=>{
              if(e.textbookCode===this.subjectId){
                    this.subjectName=e.nodeName;
                  }
            });
    ...
    methods : {
      ...
      getList() {
        ...
        // console.log(this.$store.state.commonData.resourceValue[this.subjectId]);
        // this.$store.state.commonData.resourceValue[this.subjectId].resourceType = this.searchData.resourceType;
        // this.$store.state.commonData.resourceValue[this.subjectId].sort = this.searchData.sort;
        // this.$store.state.commonData.resourceValue[this.subjectId].sortArray = this.searchData.sortArray;
      ...
      changeCode(data, nodeData){
        if(nodeData.data){
          ...
          s=s.reverse();
           this.$store.commit('SET_LOCALTREEDATA',s);
           this.searchData.textbookCode=data.textbookCode;
        }
   ...
   .el-card{
    //  margin-bottom:24px;
     ...
     .srcTypeimage {
      ...
      .RS_TYPE_PLAN{
        background: url(~assets/sourceImg/word.png) no-repeat;
      }
      .RS_TYPE_SIM{
        background: url(~assets/sourceImg/ppt.png) no-repeat;
      }
      .RS_TYPE_VIDEO{
        background: url(~assets/sourceImg/video.png) no-repeat;
      }
      .RS_TYPE_COURSEWARE{
        background: url(~assets/sourceImg/RS_TYPE_COURSEWARE.png) no-repeat;
      }
      .RS_TYPE_QUESTION{
        background: url(~assets/sourceImg/RS_TYPE_QUESTION.png) no-repeat;
      }
     .panelBody{
        font-size:12px;
        margin-left:60px;
