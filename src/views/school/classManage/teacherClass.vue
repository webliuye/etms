        ...
        <tbody v-for="(itm,idx) in allDataArray" :key="'01' + idx" v-if="itm.classData && itm.classData.length > 0">
          <tr v-for="(item,index) in itm.classData" :key="index">
            ...
            <td v-for="(items,indexs) in subjectData" :key="'04' + indexs">
              <span v-for="(imms,dex) in item.list" :key="'000'+dex"  v-if="imms.subjectId == items.value && imms.staffId && imms.staffId !== staffId" class="notTd">{{imms.staffName}}</span>
              <span v-for="(imms,dex) in item.list" :key="'000'+dex"  v-if="imms.subjectId == items.value && imms.staffId && imms.staffId === staffId" @click="deleteClass(imms)" class="allowTd">
                <el-select v-model="imms.staffId" @change="deleteClass(imms)" clearable placeholder="选择教师">
                  <el-option
                    :label="staffName"
                    :value="staffId">
                  </el-option>
              ...
              <span v-for="(imms,dex) in item.list" :key="'000'+dex"  v-if="imms.subjectId == items.value && !imms.staffId" class="emptyTd">
                <el-select v-model="imms.staffId" @change="takeClass(imms)" clearable placeholder="选择教师">
                  <el-option
                    :label="staffName"
                    :value="staffId">
                  </el-option>
  ...
  export default {
    ...
    data() {
      return {
        ...
        staffName:this.$store.state.user.userInfo.name
      };
    },
    // computed: {
  //     ...mapGetters([
